...
const panelSpanWidthWhenSlided = 21;
...
function Panel($panel, widget) {
    ...
    self.float = function() {
        ...
        self.$element.data("width", self.$element.width());
        self.$element.find(".panel-body").css("width", self.$element.width());
    ...
    self.dock = function() {
        ...
        self.$element.find(".panel-body").css("width", "");
    ...
    self._getAnimation = function() {
        var distance = self.$element.data("width") - panelSpanWidthWhenSlided;
        var direction = self.isSlided() ? "+=" : "-=";
        return self.isLeft() ? {'left': direction + distance} : {'width': direction + distance};
    };
