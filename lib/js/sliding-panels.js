...
function Panel($panel, widget) {
    ...
    self._changeStorageVariables = function(value, variable) {
        var storageName = (self.isLeft() ? "leftPanel" : "rightPanel") + variable;
    ...
    self.hasStorage = function(variable) {
        var storageName = (self.isLeft() ? "leftPanel" : "rightPanel") + variable;
        return localStorage[storageName] == "true";
    };
}
...
$.widget('ui.slidingPanels', {
    ...
    _updateStateBasedInStorage: function() {
        if (this._leftPanel.hasStorage("Floating")) {
            this._leftPanel.toggleFloating(true);
        }
        if (this._rightPanel.hasStorage("Floating")) {
            this._rightPanel.toggleFloating(true);
        }
        this._resizingCenterPanel();
        if (this._leftPanel.hasStorage("Slided")) {
            this._leftPanel.slide();
        }
        if (this._rightPanel.hasStorage("Slided")) {
            this._rightPanel.slide();
        }
    },
